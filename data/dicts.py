...
family_state = {
    'not_merried': 'Turmush qurmagan',
    'merried': 'Turmush qurgan',
    'divorced': 'Ajrashgan/Ajrashuvda',
    'single': 'Yolg\'iz(Beva)'
}

edu_grade = {
    'class_9':'Maktab(9-sinf)',
    'class_11':'Maktab(11-sinf)',
    'litsey':'Litsey',
    'collage':'Kollej',
    'hight_school':'Oliy talim',
    'magistr':'Magistratura',
    'aspirant':'Aspirantura',
    'doctorant':'Doctorantura'
}

def region_det(id) -> str:
    id = str(id)
    for reg in region:
        if reg['id']==id:
            result = reg['name_uz']
...
def district_det(id) -> str:
    id = str(id)
    for dsct in district:
        if dsct['id']==id:
            result = dsct['name_uz']
    return result
